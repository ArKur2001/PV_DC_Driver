...
#define LCD_BACKLIGHT_ON        0x08        // 1 at P3 pin
#define LCD_BACKLIGHT_OFF       0x00        // 0 at P3 pin
...
uint8_t backlight_status = LCD_BACKLIGHT_ON;
...
static void LCD_writeNibble(uint8_t nibble, uint8_t mode)
{
    uint8_t data = (nibble & 0xF0) | mode | backlight_status;
...
void set_backlight_state(enum Backligt_state eBacklight_state)
{
    ...
    {
    case OFF:
        backlight_status = LCD_BACKLIGHT_OFF;
        break;
